...
> wants an account, so it is a separate explicit module:
> `./bootstrap/setup.sh warp`. Kaku is the free/open `terminal` module.
...
./bootstrap/setup.sh terminal # Kaku
./bootstrap/setup.sh warp     # Warp only
```
